...
for chn = 1:size(s,2)
    ...
    if t(2) ~= 0
        idx = round(t_vec_r*fs) + 1 - round(t(1) * fs);  % Indices of reduced resolution in original time vector
        ERDS_{chn}.erds = erds(idx,:);
    ...
    switch lower(sig)
        ...
        case 'boot'
            for k = 1:length(f_plot)
                [~, cl, cu] = bootts(pre_erds{k}', 300, alpha);
                ...
                if smooth_length > 1
                    ERDS_{chn}.cl(:,k) = filtfilt(ones(1, smooth_length)/smooth_length, 1, cl);
                    ERDS_{chn}.cu(:,k) = filtfilt(ones(1, smooth_length)/smooth_length, 1, cu);
                else
                    ERDS_{chn}.cl(:,k) = cl;
                    ERDS_{chn}.cu(:,k) = cu;
                end
        ...
        case 'boxcox'
            ...
            for k = 1:length(f_plot)
                ...
                if lambda == 0
                    ...
                    cl_t = erds_t_m - z * ser;
                    cu_t = erds_t_m + z * ser;
                    ERDS_{chn}.cl(:,k) = exp(cl_t) - abs(offset);
                    ERDS_{chn}.cu(:,k) = exp(cu_t) - abs(offset);
                else
                   ...
                   cl_t = erds_t_m - z * ser;
                   cu_t = erds_t_m + z * ser;
                   ERDS_{chn}.cl(:,k) = (cl_t * lambda + 1).^(1/lambda) - abs(offset);
                   ERDS_{chn}.cu(:,k) = (cu_t * lambda + 1).^(1/lambda) - abs(offset);
                end;
            end;
